...
import static com.nio.redis.framework.client.protocol.CommandType.MULTI;
 ...
 * share one {@link Redis} provided they avoid blocking and transactional
...
public class Redis<K, V> {
    ...
    public RedisKeyValue<K, V> blpop(long timeout, K... keys) {
    ...
    public RedisKeyValue<K, V> brpop(long timeout, K... keys) {
        long timeout2 = (timeout == 0 ? Long.MAX_VALUE : max(timeout, unit.toSeconds(this.timeout)));
